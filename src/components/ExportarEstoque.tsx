    ...
    try {
      ...
      const dadosProdutos = produtos.map((produto, index) => {
        ...
        return {
          ...
          '📊 ESTOQUE ATUAL': sanitizeForExcel(`${Math.round(estoqueAtual)} ${produto.unidade_conteudo}`),
          '📈 MÉDIA ESTOQUE': sanitizeForExcel(`${Math.round(mediaEstoque)} ${produto.unidade_conteudo}`),
    ...
    try {
      ...
      const dadosHistorico = contagens.map((contagem, index) => {
        ...
        return {
          ...
          '🧮 TOTAL CALCULADO': sanitizeForExcel(`${Math.round(contagem.quantidade_total)} ${produto?.unidade_conteudo || '-'}`),
