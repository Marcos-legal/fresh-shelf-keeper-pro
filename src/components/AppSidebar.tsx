...
export function AppSidebar() {
  ...
  return (
    <aside
      className={cn(
        "fixed inset-y-0 left-0 z-40 hidden transition-[width] duration-200 ease-linear lg:block",
      ...
      <Sidebar className="h-full w-full border-r border-slate-200 bg-white text-slate-700 shadow-[2px_0_12px_rgba(15,23,42,0.04)]">
        <SidebarHeader className={cn("border-b border-slate-100 bg-white py-4", state === "expanded" ? "px-4" : "px-2")}>
          ...
          <button
            ...
            className={cn(
              "mt-3 flex h-8 w-full items-center rounded-md text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-800",
              state === "expanded" ? "justify-end px-2" : "justify-center"
            )}
          >
                  ...
                  {section.items.map((item) => {
                    ...
                    return (
                      <SidebarMenuItem key={item.title}>
                        ...
                        >
                          <Link to={item.url} className="flex w-full items-center justify-between gap-2">
                            <span className="flex min-w-0 items-center gap-2.5">
                              <item.icon className={cn("h-4 w-4 shrink-0", active ? "text-primary" : "text-slate-400")} />
                              <span className="truncate text-[13px] group-data-[collapsible=icon]:hidden">{item.title}</span>
                            </span>
                            {count > 0 && (
                              <span className="inline-flex h-[19px] min-w-[19px] shrink-0 items-center justify-center rounded-full bg-slate-100 px-1 text-[9px] font-semibold text-slate-500 group-data-[collapsible=icon]:hidden">
                                {count > 999 ? "999+" : count}
                              </span>
                            )}
                          </Link>
