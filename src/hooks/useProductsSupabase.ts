...
export function useProductsSupabase() {
  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      if (error) { console.error('Error loading products:', error); toast({ title: 'Erro ao carregar produtos', description: error.message || 'Não foi possível carregar os produtos do banco de dados.', variant: 'destructive' }); return; }
  ...
  const addProduct = async (data: ProductFormData) => {
    ...
    try {
      ...
      if (data.validade?.trim()) {
        ...
        expiryDate = `${parsedDate.getFullYear()}-${String(parsedDate.getMonth() + 1).padStart(2, '0')}-${String(parsedDate.getDate()).padStart(2, '0')}`;
      }
      const manufactureDate = toIsoDate(data.dataFabricacao);
      const openingDate = toIsoDate(data.dataAbertura);
      let useByDate = null;
      if (openingDate && data.diasParaVencer && data.diasParaVencer > 0) {
        const [year, month, day] = openingDate.split('-').map(Number); const useBy = new Date(year, month - 1, day + data.diasParaVencer);
        useByDate = `${useBy.getFullYear()}-${String(useBy.getMonth() + 1).padStart(2, '0')}-${String(useBy.getDate()).padStart(2, '0')}`;
      }
      const productData = { name: data.nome || '', lot: data.lote || '', brand: data.marca || '', manufacture_date: manufactureDate, expiry_date: expiryDate, opening_date: openingDate, days_valid: data.diasParaVencer || 0, use_by_date: useByDate, storage: data.localArmazenamento || 'ambiente', responsible: data.responsavel || '', status: 'active', user_id: user.id, empresa_id: activeEmpresaId, preco_custo: data.precoCusto ?? null };
  ...
  const updateProduct = async (id: string, data: Partial<ProductFormData>) => {
    ...
    try {
      ...
      if (data.dataFabricacao !== undefined) updateData.manufacture_date = toIsoDate(data.dataFabricacao);
      ...
      if (data.dataAbertura !== undefined) updateData.opening_date = toIsoDate(data.dataAbertura);
      if (data.dataAbertura !== undefined && toIsoDate(data.dataAbertura) && daysValid && daysValid > 0) {
        const openingDate = toIsoDate(data.dataAbertura)!; const [year, month, day] = openingDate.split('-').map(Number); const useBy = new Date(year, month - 1, day + daysValid);
      ...
      if (!updatedRows || updatedRows.length === 0) { toast({ title: 'Produto não atualizado', description: 'O produto não foi encontrado ou não há permissão para alterá-lo.', variant: 'destructive' }); return; }
      await loadProducts(); toast({ title: 'Produto atualizado', description: 'O produto foi atualizado com sucesso.' });
  ...
  const updateDataAbertura = async (id: string, novaData: Date) => {
    ...
    try {
      ...
      const { error } = await supabase.from('products').update({ opening_date: dateStr, use_by_date: useByDate }).eq('id', Number(id)).eq('empresa_id', activeEmpresaId);
